...
import {fetchJSON, fetchWithTimeout, showNotification} from "./utils.js";
...
class HelperApp {
    ...
    async getPort() {
        if (this.port) {
            ...
            this.port = new Promise(async (resolve, reject) => {
                const port = browser.runtime.connectNative("slideshow_helper");
    ...
    async probe(verbose = false) {
        ...
        if (!presents && verbose)
            showNotification("Can not connect to the helper application.")
